...
void display_bitbang_spi() {
    for (int i = 0; i < 8; i++) {
        uint16_t value = msg[i];
        
        gpio_put(SPI_7SEG_CSn, 0);
        sleep_us(10);
        
        for (int bit = 15; bit >= 0; bit--) {
            gpio_put(SPI_7SEG_TX, (value >> bit) & 1);
            sleep_us(1);
...
void display_print() {
    ...
    spi_write16_blocking(spi, msg, 8);
}
...
void display_init_dma() {
    ...
    channel_config_set_chain_to(&c, dma_channel);
    
    volatile void *spi_dr = &spi_get_hw(spi)->dr;
